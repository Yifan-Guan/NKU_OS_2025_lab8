...
text
Level 1: 缓冲区访问互斥
    └── 使用mutex信号量保护读写指针和data_count
        （确保指针操作的原子性）

Level 2: 条件等待协调
    ├── 读进程等待条件：data_count > 0 或 write_closed
    └── 写进程等待条件：buffer_size - data_count > 0 且 read_closed为false
...
// 多个进程同时写入同一管道
// 需要保证：
// 1. 小于PIPE_BUF的写入具有原子性
// 2. 数据不会交错（单个进程的写入是连续的）
// 3. 写阻塞时的公平唤醒
场景C：读写同时发生
c
// 读写操作并发执行
